...
def onTrackbarChange(max_slider):
    ...
    lines = cv2.HoughLinesP(edges, 2, np.pi / 180.0, 50, minLineLength=200, maxLineGap=50)
    ...
    for line in lines:
        ...
        if (x2-x1,y2-y1) > max :
            ...
            uzunluk = (x2-x1)**2+(y2-y1)**2
            uzunluk =  sqrt(uzunluk)
            print(uzunluk)

    dstd = cv2.resize(dst,(600,600))
    edgess = cv2.resize(dst, (600,600))
    cv2.imshow("Result Image", dstd)
    cv2.imshow("Edges", edgess)
